...
test("snapshot repair is deterministic and preserves current observations", () => {
  ...
  const later = "2026-08-20T03:00:00.000Z";
  ...
  assert.equal(first.summary.networkPerformance.imputed, 2);
  assert.equal(first.summary.validators.imputed, 2);
  assert.equal(first.summary.medianTransactionFee.imputed, 2);
...
test("available RPC performance evidence is recovered before interpolation", () => {
  ...
  assert.equal(result.summary.networkPerformance.imputed, 1);
  assert.equal(result.snapshot.network.performance.history[1].recoveredFrom, NETWORK_RECOVERY_SOURCE);
  assert.equal(result.snapshot.network.performance.history[2].imputed, true);
