...
- **CoinGecko always-on comparison and fallback:** CoinGecko's keyless public price and daily history are attempted independently on their hourly schedule, including while DefiLlama is healthy, and retained as a separately identified comparison domain. If the current quote and chart contain the same provider timestamp, the current quote is authoritative for that point. If DefiLlama fails while CoinGecko succeeds, CoinGecko may replace the complete canonical price domain; individual fields or histories from the two providers are never spliced together.
...
- Upcoming developments are current non-live cards from the official Solana upgrades hub. Recognized stages are `In Development`, `Pending Feature Activation`, `Partially Activated`, and `Action Required`; the two activation stages share the canonical `pending_activation` lifecycle state while retaining the exact official label. Unknown stages fail the collector. Official detail pages provide SIMD links. The current contract asserts Alpenglow/SIMD-0326 and Reduced Slot Times/SIMD-0525 while they remain officially upcoming.
...
- Historical continuity repair is limited to gaps that would otherwise render as disconnected lines: strictly longer than six hours for the hourly network, validator, and median-fee histories, and strictly longer than 36 hours for the six-hour Tokens.xyz history. Shorter missing intervals remain unfilled. For a qualifying gap, the repair inserts `round(gap / cadence) - 1` points at evenly spaced timestamps between two non-imputed bounding points and never extrapolates before the first or after the last point. Numeric fields use linear interpolation; integer counts are rounded; lamport strings use integer arithmetic; derived validator percentages are recomputed from the interpolated stakes. A single gap larger than its entire retained history capacity is not treated as a standard repair and stops publication for manual review.
- Every estimated row carries `imputed: true` in canonical JSON and appears as “Estimated” in the explorer table and tooltip. The main chart line keeps a single continuous style to avoid visual clutter. Every snapshot-producing process runs the shared repair pass before publication; it first removes prior estimates and deterministically rebuilds them from non-imputed points, so estimates never compound. Unrecognized post-process failures still block publication and enter a reserved notification hook whose delivery is currently disabled.
- Three September 22 network-performance points were recovered before the RPC's recent-sample window expired. Their five one-minute inputs, ending slots, and recomputed values are checked in at `scripts/history-gap-recovery-results.json`; they carry `recoveredFrom: "solana_rpc_performance_samples"`, not `imputed`. Older performance samples, historical vote-account snapshots, absent scheduled median-fee samples, and past Tokens.xyz snapshots were not available with their original point-in-time semantics. They remain missing unless a gap crosses the visual-continuity thresholds above.
